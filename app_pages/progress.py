...
if st.session_state.get("in_sis") and st.session_state.get("snowpark_session"):
    st.subheader("My CoCo usage (live data)")
    try:
        ...
        if not usage_df.empty:
            ...
            st.caption("No CoCo usage data found for the last 30 days.")
    except Exception as e:
        st.caption(f"Usage data unavailable: {e}")
else:
    st.caption("CoCo usage data is available when running in Snowflake (Snowsight).")
